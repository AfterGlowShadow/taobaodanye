...
class FileTools {
	...
	function forcemkdir($path) {
		if (!file_exists($path)) {
			FileTools::forcemkdir(dirname($path));
			mkdir($path, 0777);
		}
	}
	...
	function cleardir($dir, $forceclear = false) {
		...
		while ($entry = $directory->read()) {
			...
			if (is_file($filename)) {
				...
				chmod($filename, 0777);
				FileTools::cleardir($filename, $forceclear);
				rmdir($filename);
			}
		}
		$directory->close();
			...
			while ($file = readdir($curdir)) {
				if ($file != '.' && $file != '..') {
					...
					if (is_file($srcfile)) {
						...
						FileTools::copydir($srcfile, $dstfile);
					}
				}
			}
			closedir($curdir);
	...
	function downloadfile($filepath, $filename = '') {
		...
		$dateline = time();
		FileTools::hheader('date: ' . gmdate('d, d m y h:i:s', $dateline) . ' gmt');
		FileTools::hheader('last-modified: ' . gmdate('d, d m y h:i:s', $dateline) . ' gmt');
		FileTools::hheader('content-encoding: none');
		FileTools::hheader('content-disposition: attachment; filename=' . $filename);
		FileTools::hheader('content-type: ' . $filetype);
		FileTools::hheader('content-length: ' . $filesize);
		FileTools::hheader('accept-ranges: bytes');
		if (!@empty($_SERVER['HTTP_RANGE'])) {
			list($range) = explode('-', (str_replace('bytes=', '', $_SERVER['HTTP_RANGE'])));
			$rangesize = ($filesize - $range) > 0 ? ($filesize - $range) : 0;
			FileTools::hheader('content-length: ' . $rangesize);
			FileTools::hheader('http/1.1 206 partial content');
			FileTools::hheader('content-range: bytes=' . $range . '-' . ($filesize - 1) . '/' . ($filesize));
		}
	...
	function createaccessfile($path) {
		if (!file_exists($path . 'index.htm')) {
			$content = ' ';
			FileTools::writetofile($path . 'index.htm', $content);
		}
		if (!file_exists($path . '.htaccess')) {
			$content = 'Deny from all';
			FileTools::writetofile($path . '.htaccess', $content);
		}
	}
	...
	function getdirsize($filedir) {
		...
		while ($filename = readdir($handle)) {
			if ('.' != $filename && '..' != $filename) {
				$totalsize += is_dir($filedir . '/' . $filename) ? FileTools::getdirsize($filedir . '/' . $filename) : (int)filesize($filedir . '/' . $filename);
			}
		}
		return $totalsize;
	}

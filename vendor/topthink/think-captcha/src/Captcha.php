...
class Captcha
{
    ...
    {
        $key = $this->authcode($this->seKey) . $id;log_record('---#### check = ' . $key);
        // 验证码不能为空
        $secode = Session::get($key, '');
        if (empty($code) || empty($secode)) {log_record('---#### check secode = ' . json_encode_u($secode));
            return false;
        }
        // session 过期
	    ...
	    log_record('---#### check = ' . $key . ' ' . $this->authcode(strtoupper($code)) . ' ' . $secode['verify_code']);
    ...
    public function entry($id = '')
    {
	    ...
	    log_record('---#### entry = ' . $key . $id);
	    $secode = Session::get($key . $id, '');
	    log_record('---#### entry secode= ' . json_encode_u($secode));
        ob_start();
        // 输出图像
        imagepng($this->im);
